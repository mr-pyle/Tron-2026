...
bots_path = os.path.abspath(BOTS_DIR)
...
class TronApp:
    ...
    def refresh_bot_list(self):
        ...
        current_files = sorted([f[:-3] for f in os.listdir(BOTS_DIR) if f.endswith('.py') and f != 'main.py' and f != 'network_test.py'])
        new_vars = {}
        ...
        for index, f in enumerate(current_files):
            ...
            is_safe, reason = is_bot_safe(os.path.join(BOTS_DIR, f"{f}.py"))
    ...
    def start_visual_match(self):
        ...
        self.bot_snapshots = {}
        for bot in selected:
            bot_filepath = os.path.join(BOTS_DIR, f"{bot}.py")
    ...
    def process_tick(self, visual=True):
        ...
        def fetch_move(p):
            try:
                ...
                # USE ENGINE.PY TO CALCULATE POSITION
                new_coord = calculate_new_pos(p['pos'], move)
                return p['id'], new_coord, None
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(alive)) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if result == "ERROR":
                    ...
                    # We let the engine handle the actual death logic now
                else:
                    intended_moves[p['id']] = result

        # --- PHASE 2 & 3: ENGINE RESOLVES COLLISIONS ---
        died_this_turn = resolve_collisions(
            self.grid_dim, alive, intended_moves, 
            self._engine_board, self._dead_player_ids, current_rank_score
        )

        # --- PHASE 4: UPDATE THE VISUALS BASED ON ENGINE RESULTS ---
        if visual:
            for p in alive:
                # If they died this turn, paint them dead
                if p['id'] in died_this_turn:
                    self.canvas.itemconfig(f"p{p['id']}", fill=get_dead_color(p['color']))
                    self.canvas.delete(f"name_{p['id']}")
                # If they are still alive, draw their new trail segment
                elif p['alive']:
                    nx, ny = p['pos']
                    bright_head = get_fade_color(p['color'], 0)
    ...
    def start_tournament(self):
        ...
        def pool_manager():
            ...
            for bot_name in selected:
                try:
                    bot_filepath = os.path.join(BOTS_DIR, f"{bot_name}.py")
                    with open(bot_filepath, 'r', encoding='utf-8') as f:
                        code_snapshots[bot_name] = f.read()
                except Exception:
                    pass
